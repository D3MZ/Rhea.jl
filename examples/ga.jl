using Persephone, Statistics, Dates, Random, Logging, JSON
import Persephone: GeneticAgent, Node, OpNode, FeatureNode, ConstNode, dict, node_from_dict, State, Reward, run!, nlv, DataBroker, mid, targetorders
...
mutate(root::Node) = 
    swap(root, sample(root), rand() < 0.5 ? Persephone.random_node(1, 4) : Persephone.random_node(3, 3))
...
function Persephone.Reward(r::TurnoverTracker, s1::State, s2::State)
...
function advance(pop, fits)
    ...
    best = argmax(fits)
    next[1] = Persephone.simplify(pop[best])
    
    for i in 2:length(pop)
        child = crossover(select_node(pop, fits), select_node(pop, fits))
        child = rand() < 0.3 ? mutate(child) : child
        next[i] = Persephone.simplify(child)
    end
    return next, best
end

function resizepop(pop, size)
    pop === nothing && return Node[Persephone.random_node(1, 3) for _ in 1:size]
    n = length(pop)
    n >= size && return pop[1:size]
    needed = size - n
    vcat(pop, Node[Persephone.random_node(1, 3) for _ in 1:needed])
end
