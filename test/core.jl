...
@testset "Path Connectivity Checks" begin
    ...
    @test_throws ErrorException Rhea.build_paths(insts, pairs, account_currency)

    try
        Rhea.build_paths(insts, pairs, account_currency)
    catch e
    ...
    paths = Rhea.build_paths(insts2, pairs2, account_currency)
    @test size(paths) == (2, 4)
